...
export default function Disease() {
  ...
  if (!disease) {
    return (
      <div className="p-6 text-center text-green-700 font-semibold bg-green-50 min-h-screen flex items-center justify-center">
        <div className="max-w-md">
          ...
          <button 
            onClick={() => navigate('/')} 
            className="px-4 py-2 bg-green-600 text-green-50 rounded hover:bg-green-700 transition-colors"
          >
            Return to Home
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col md:flex-row bg-green-50 min-h-screen font-serif m-0 p-0">

      {/* Sidebar Toggle Button */}
      <button 
    className="md:hidden fixed top-0 right-4 z-20 bg-green-500 text-white w-10 h-10 rounded-full shadow-lg flex items-center justify-center mt-2"
  ...
  >
    {isSidebarOpen ? '✕' : '☰'}
      ...
      <div className="w-full md:fixed md:w-56 lg:w-64 bg-green-100 p-4 shadow-inner flex-shrink-0 overflow-y-auto md:h-screen">
        <div className="sticky top-4">
          <h3 className="text-xl font-semibold text-green-800 mb-6 border-b border-green-300 pb-2">
            Treatment Journey
          </h3>
      ...
      <div className="flex-grow pt-0 pb-4 px-4 md:pt-8 md:pb-8 md:px-8 lg:pt-10 lg:pb-10 lg:px-10 sm:pt-0 max-w-6xl mx-auto md:ml-56 lg:ml-64 overflow-y-auto">
        ...
        {/* Header */}
        <div className="flex flex-col md:flex-row items-center md:items-start mb-10 border-b-2 border-green-200 pb-8">
          <img
            src={disease.image_url || '/api/placeholder/200/200'}
            alt={disease.alt_text || 'Ayurvedic Remedy'}
            className="w-32 h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 object-cover rounded-full border-4 border-green-300 shadow-lg mb-4 md:mb-0 md:mr-8"
            onError={(e) => {
              e.target.src = '/api/placeholder/200/200';
            }}
          />
          <div className="text-center md:text-left">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-green-800 font-serif">{disease.alt_text || 'Ayurvedic Remedy'}</h1>
            <p className="text-lg md:text-xl lg:text-2xl text-green-700 mt-2">{disease.disease_name}</p>
            {disease.brief_text && (
              <p className="text-base md:text-lg italic text-green-600 mt-3 max-w-3xl">{disease.brief_text}</p>
            )}
          </div>
        </div>
        ...
        {disease.overview_text && (
          <section ref={overviewRef} className="mb-10 scroll-mt-4">
            <h2 className="text-2xl md:text-3xl font-semibold text-green-800 mb-4 border-l-4 border-green-500 pl-4">Overview</h2>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <p className="text-lg md:text-xl text-green-900 leading-relaxed">{disease.overview_text}</p>
            </div>
          </section>
        )}
        ...
        {disease.keyfacts?.length > 0 && disease.keyfacts.some(f => f.title || f.values?.[0]) && (
          <section ref={keyFactsRef} className="mb-10 scroll-mt-4">
            <h2 className="text-2xl md:text-3xl font-semibold text-green-800 mb-4 border-l-4 border-green-500 pl-4">Key Facts</h2>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <ul className="space-y-3">
                {disease.keyfacts.map((fact, idx) => (
                  <li key={idx} className="flex items-start">
                    ...
                    <div>
                      <span className="font-semibold text-green-800 text-lg md:text-xl">{fact.title}: </span>
                      <span className="text-green-700 text-lg md:text-xl">{fact.values?.join(', ')}</span>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        {disease.symptoms?.length > 0 && (
          <section ref={symptomsRef} className="mb-10 scroll-mt-4">
            <h2 className="text-2xl md:text-3xl font-semibold text-green-800 mb-4 border-l-4 border-green-500 pl-4">Symptoms</h2>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {disease.symptoms.map((symptom, idx) => (
                  <li key={idx} className="flex items-start">
                    <span className="text-green-500 text-xl mr-3">•</span>
                    <span className="text-green-700 text-lg">{symptom}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        {(disease.diagnosis_title || disease.diagnosis_text) && (
          <section ref={diagnosisRef} className="mb-10 scroll-mt-4">
            <h2 className="text-2xl md:text-3xl font-semibold text-green-800 mb-4 border-l-4 border-green-500 pl-4">
              {disease.diagnosis_title || 'Diagnosis'}
            </h2>
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <p className="text-lg md:text-xl text-green-900 leading-relaxed">{disease.diagnosis_text}</p>
            </div>
          </section>
        )}
        ...
        {(disease.prevention_title || disease.prevention_text) && (
          <section ref={preventionRef} className="mb-10 scroll-mt-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <p className="text-lg md:text-xl text-green-900 leading-relaxed">{disease.prevention_text}</p>
            </div>
          </section>
        )}
          ...
          <section ref={specialistsRef} className="mb-10 scroll-mt-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              {disease.specialist_description && (
                <p className="text-lg md:text-xl text-green-900 leading-relaxed mb-4">{disease.specialist_description}</p>
              )}
                ...
                <ul className="space-y-2">
                  {disease.specialists.map((spec, idx) =>
                    spec && (
                      <li key={idx} className="flex items-start">
                        <span className="text-green-500 text-xl mr-3">•</span>
                        <span className="text-green-700 text-lg">{spec}</span>
                      </li>
                    )
                  )}
                </ul>
        ...
        {(disease.home_care_title || disease.home_care_text) && (
          <section ref={homeCareRef} className="mb-10 scroll-mt-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <p className="text-lg md:text-xl text-green-900 leading-relaxed">{disease.home_care_text}</p>
            </div>
          </section>
        )}
        ...
        {(disease.dos?.length || disease.donts?.length) && (
          <section ref={dosAndDontsRef} className="mb-10 scroll-mt-4">
            <h2 className="text-2xl md:text-3xl font-semibold text-green-800 mb-4 border-l-4 border-green-500 pl-4">Do's and Don'ts</h2>
        ...
        {(disease.alternative_therapies_title || disease.alternative_therapies_text) && (
          <section ref={alternativeTherapiesRef} className="mb-10 scroll-mt-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-green-200">
              <p className="text-lg md:text-xl text-green-900 leading-relaxed">{disease.alternative_therapies_text}</p>
            </div>
          </section>
        )}
      </div>
