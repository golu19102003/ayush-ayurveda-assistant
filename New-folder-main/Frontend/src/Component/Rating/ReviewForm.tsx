...
const ReviewForm = () => {
  ...
  const renderStars = (rating: number) => (
    <div className="flex justify-center mt-2">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={`text-lg ${i < rating ? 'text-green' : 'text-gray-300'}`}>★</span>
      ))}
    </div>
  );
  ...
  return (
    <div className="w-full flex flex-col items-center bg-green-50 from-green-100 via-green-50 to-green-100 transition-all text-gray-900 py-16 px-4 font-sans">
      <h2 className="text-4xl sm:text-5xl font-bold text-green-600 text-center mb-10 tracking-wide">
      ...
      {reviews.length > 0 && (
        <>
          <div className="relative w-full max-w-4xl flex items-center justify-center">
            <button
              onClick={goToPrev}
              className="absolute left-0 sm:left-4 bg-green-500 hover : bg-green-600 text-white w-10 h-10 sm:w-12 sm:h-12 rounded-full shadow-md flex items-center justify-center z-10 transition"
            >
              ←
            </button>

            <div className="w-full max-w-lg p-6 bg-green-50 border border-green-100 rounded-xl shadow-md text-center transition-all duration-300">
            ...
            <button
              onClick={goToNext}
              className="absolute right-0 sm:right-4 bg-green-500 hover:bg-green-600 text-white w-10 h-10 sm:w-12 sm:h-12 rounded-full shadow-md flex items-center justify-center z-10 transition"
            >
              →
            </button>
          </div>
      ...
      {/* Feedback CTA */}
      <div className="mt-10 bg-green-50 border border-green-100 p-6 rounded-lg text-center shadow-md w-full max-w-lg">
        <p className="text-sm sm:text-base text-green-700 mb-3">
          Want to share your experience? Leave a review!
        </p>
        <button
          onClick={() => setShowFeedbackForm(true)}
          className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm rounded-lg shadow transition"
        >
          Give Feedback
        </button>
      </div>
      ...
      {showFeedbackForm && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div
            ...
            className="bg-white text-gray-800 rounded-xl p-6 w-full max-w-md shadow-lg relative"
          >
            <button 
              onClick={() => setShowFeedbackForm(false)}
              className="absolute top-4 right-4 text-gray-500 hover:text-red-500 text-lg rounded-full w-6 h-6 flex items-center justify-center hover:bg-gray-100 transition-colors duration-200"
            >
              ✖
            </button>
            <h3 className="text-xl font-bold mb-4 text-green-700 pr-6">Share Your Experience</h3>
            ...
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div className="flex flex-col items-center">
                <label 
                  ...
                  className="px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-sm rounded-md cursor-pointer transition-colors"
                >
              ...
              <div className="flex justify-center space-x-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className={`text-2xl ${
                      star <= formData.rating ? 'text-green -400' : 'text-gray-400'
                    }`}
                  >
                    ★
                  </button>
