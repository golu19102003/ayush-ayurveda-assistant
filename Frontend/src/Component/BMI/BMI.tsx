...
export default function BMICalculator() {
  ...
  const [yogaExercises, setYogaExercises] = useState<string[]>([]);
  ...
  const getBMICategory = (bmiValue: string | number): BMICategory => {
  ...
  const getYogaExercises = (category: BMICategory, heightConcern = false): string[] => {
    if (heightConcern) {
      return [
  ...
  const getRecommendedCalories = (category: BMICategory, currentCalories: string): string => {
  ...
  const getRecommendation = (category: BMICategory, heightValue: string, heightUnitValue: HeightUnit): RecommendationResult => {
    ...
    if ((heightUnitValue === 'cm' && heightNum < 160) || 
        (heightUnitValue === 'm' && heightNum < 1.6) || 
        (heightUnitValue === 'ft' && heightNum < 5.3) || 
        (heightUnitValue === 'in' && heightNum < 63)) {
      heightConcern = true;
    }
  ...
  const renderBmiAvatar = (): React.ReactElement | null => {
  ...
  const renderUserAvatar = (): React.ReactElement | null => {
    if (profileImage) {
      return (
        <div className="relative w-32 h-32 mx-auto mb-4">
          <img 
            src={profileImage} 
            alt="User profile" 
            className="w-full h-full object-cover rounded-full border-4 border-green-200 shadow-md"
          />
          <button 
            onClick={removeImage}
            className="absolute -top-2 -right-2 bg-green-500 text-white rounded-full p-1 shadow-md hover:bg-green-100 transition-colors"
          >
            <X size={16} />
          </button>
        </div>
    ...
    return (
      <div className="relative w-32 h-32 mx-auto mb-4 bg-green-100 rounded-full flex items-center justify-center border-4 border-green-200 shadow-md">
  ...
  const getBmiColor = (category: BMICategory): string => {
    switch (category) {
      case 'underweight': return 'text-green-300';
      case 'normal': return 'text-green-600';
      case 'overweight': return 'text-orange-500';
      case 'obese': return 'text-green-500';
      default: return 'text-gray-800';
    }
  };

  return (
    <div className="flex flex-col md:flex-row w-full h-full bg-green-50 rounded-lg overflow-hidden shadow-lg border-t border-green-200">
      {/* Left Side - Avatar and BMI Result */}
      <div className="w-full md:w-1/3 bg-green-50 p-6 flex flex-col border-r border-green-200">
        ...
        {isSubmitted ? renderBmiAvatar() : (
          <div className="mt-4 p-4 bg-green-100 rounded-lg animate-pulse border border-green-200">
            <p className="text-green-800 text-sm text-center">Complete the form to see your BMI results</p>
          </div>
        )}
        
        {isSubmitted && (
          <div className="mt-4 p-4 bg-green-100 rounded-lg animate-fade-in border border-green-200">
            <h3 className="font-semibold text-green-800 mb-2 flex items-center">
              <Award className="mr-2" size={18} />
              Hello, {name}!
            </h3>
            <p className="text-green-900 text-sm">{recommendation}</p>
          </div>
        )}
      </div>
      
      {/* Right Side - Form and Results */}
      <div className="w-full md:w-2/3 bg-green-50 p-6">
        {!isSubmitted ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <h1 className="text-2xl font-bold text-green-800 mb-6">BMI Calculator</h1>
            
            <div className="form-group">
              <label htmlFor="user-name" className="flex items-center text-green-800 mb-2">
              ...
              <input
                ...
                className="w-full p-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:border-green-500 bg-green-50"
                required
              />
            </div>
            
            <div className="form-group mb-6">
              <label htmlFor="profile-upload" className="flex items-center text-green-800 mb-2">
              ...
              <div className="flex items-center">
                ...
                <label
                  ...
                  className="cursor-pointer bg-green-100 hover:ring-green-200 text-green-700 font-medium py-2 px-4 rounded-md transition-colors duration-200 flex items-center border border-green-300"
                >
                ...
                {!useDefaultAvatar && (
                  <button
                    type="button"
                    onClick={removeImage}
                    className="ml-2 text-red-600 hover:text-green-700"
                  >
                    Remove
                  </button>
                )}
              </div>
              <p className="text-xs text-green-700 mt-1">Upload your photo or use our default avatar</p>
            </div>
            
            <div className="form-group">
              <label htmlFor="user-weight" className="flex items-center text-green-800 mb-2">
              ...
              <div className="flex">
                <input
                  ...
                  className="w-full p-2 border border-green-300 rounded-l-md focus:outline-none focus:ring-2 focus:border-green-500 bg-green-50"
                  required
                  min="1"
                  step="0.1"
                />
            ...
            <div className="form-group">
              <label htmlFor="user-height" className="flex items-center text-green-800 mb-2">
              ...
              <div className="flex">
                ...
                <select
                  ...
                  className="bg-green-100 border border-green-300 rounded-r-md px-3 focus:outline-none text-green-800"
            ...
            <div className="form-group">
              <label htmlFor="calorie-intake" className="flex items-center text-green-800 mb-2">
            ...
            <div className="p-4 bg-white rounded-lg shadow-sm border border-green-200">
              ...
              <ul className="list-disc pl-6 space-y-1">
                {yogaExercises.map((exercise, index) => (
                  <li key={index} className="text-green-900">{exercise}</li>
                ))}
              </ul>
            </div>
