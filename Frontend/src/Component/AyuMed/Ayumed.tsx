...
const Ayumed = () => {
  ...
  return (
    <div className="min-h-screen bg-green-50 py-10 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        {/* Header with subtle animation */}
        <div className="text-center mb-10 animate-fade-in">
          <h1 className="text-3xl md:text-5xl font-bold text-green-800 mb-3 animate-title">
            Find Ayurvedic Treatments
          </h1>
          <p className="text-green-700 max-w-lg mx-auto animate-subtitle">
        ...
        <div className="max-w-md mx-auto mb-12 relative animate-slide-up">
          <div className="relative">
            <input
              ...
              className="w-full py-3 pl-12 pr-4 rounded-full border border-green-200 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-300 focus:ring-opacity-50 shadow-md text-green-900 transition-all duration-300"
            />
          ...
          {isSearchFocused && searchTerm && (
            <div className="absolute z-10 mt-1 w-full bg-white rounded-lg shadow-lg border border-green-100 py-2 max-h-64 overflow-y-auto animate-expand">
              {filteredCities.length > 0 ? (
                filteredCities.map((city) => (
                  <div
                    key={`suggestion-${city}`}
                    className="px-4 py-2 hover:bg-green-50 cursor-pointer transition-colors duration-200"
                    onClick={() => {
                      setSearchTerm(city);
                      handleClick(city);
                    }}
                ...
                <div className="px-4 py-2 text-green-500 italic">
                  No cities found
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
          {(searchTerm ? filteredCities : cities).map((city, index) => (
            ...
            >
              <div 
                className={`h-40 flex items-center justify-center rounded-xl shadow-lg ${cityColors[index % cityColors.length]} overflow-hidden card-glow`}
              >
        ...
        {searchTerm && filteredCities.length === 0 && (
          <div className="text-center py-10 animate-fade-in">
            ...
            <button
              ...
              className="mt-4 px-6 py-2 bg-green-100 text-green-700 rounded-full hover:bg-green-200 transition-all duration-300 hover:shadow-md"
            >
              Clear Search
            </button>
          </div>
      ...
      <style jsx>{`
        ...
        @keyframes subtle-pulse {
          0% { box-shadow: 0 5px 15px rgba(74, 222, 128, 0.3); }
          50% { box-shadow: 0 8px 20px rgba(74, 222, 128, 0.5); }
          100% { box-shadow: 0 5px 15px rgba(74, 222, 128, 0.3); }
