  ...
  const renderStars = (rating) => {
    ...
    return (
      <div className="flex items-center">
        ...
        <span className="ml-2 text-sm text-green-700">{rating.toFixed(1)}</span>
      </div>
    );
  };

  return (
    <div className="w-full flex justify-center items-center py-16 bg-green-50 from-green-100 via-green-100 to-green-100">
      <div className="w-full max-w-4xl p-6 bg-green-100 rounded-xl text-green-800">
        <div className="flex flex-col md:flex-row w-full">
          ...
          <div className="w-full md:w-1/2 p-4 flex flex-col justify-between">
            <div>
              <h2 className="text-2xl font-bold text-green-600 mb-2">{doctor.name}</h2>
              <div className="mb-2 text-green-500 font-medium">
                {doctor.specialist}
              </div>
              <p className="text-green-700 mb-3 text-sm">{doctor.description}</p>
              <div className="mb-3">
                {renderStars(doctor.rating)}
              </div>
              <div className="flex items-start mb-4">
                <svg className="w-4 h-4 text-green-400 mt-1 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <p className="text-xs text-green-500">{doctor.location}</p>
              </div>
            </div>
      ...
      className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-md flex items-center justify-center transition-all duration-300"
    >
      <svg
          ...
          {doctors.map((_, index) => (
            <button
              ...
              className={`mx-1 w-3 h-3 rounded-full transition-all duration-300 ${
                index === currentDoctorIndex ? "bg-green-500" : "bg-green-300"
              }`}
